...
export default function MovieModal(props) {
    ...
    React.useEffect(() => {
        fetch("/api/poster/" + props.movie.imdb_title_id)
            .then((res) => res.json())
                ...
                <Box sx={style}>
                    <Grid container spacing={3}>
                        <Grid item xs={9}>
                            <Grid alignItems="center" container py={1} justifyContent="space-between">
                                <Grid align="left" item>
                                    ...
                                    <Grid item sx={{mt: 2}}>
                                        <Grid container spacing={1}>
                                            {props.movie.language ? props.movie.language.map(x => {
                                                return (
                                                    <Grid item key={props.movie.imdb_title_id + "_" + x}>
                                                        <GenreChip genre={x}/>
                                                    </Grid>
                                    ...
                                    props.movie.plot.length > 350 ? (
                                            <div>
                                                <Typography textAlign="justify" variant="body2" sx={{mt: 2}}>
                                                    {!fullPlot ? props.movie.plot.substring(0, 350) + "..."
                                                        : props.movie.plot}
                                                    <a style={{
                                                        textDecoration: "underline",
                                                        color: "blue",
                                                        cursor: "pointer"
                                                    }} onClick={handleReadMore}>Read {fullPlot ? "Less" : "More"}</a>
                                                </Typography>
                                            </div>
                                        ) :
                                        <Typography textAlign="justify" variant="body2" sx={{mt: 2}}>
                            ...
                            <Grid container direction="column" spacing={3}>
                                {props.movie.personal ? props.movie.personal.map(x => {
                                    return (
                                        <Grid key={x.id} item>
                                            <PersonCard person={x}/>
                                        </Grid>
                        ...
                        <Grid xs={3} item align="center">
                            {!data ? 'Loading...' : <img alt={props.movie.original_title + " poster"} className="poster" src={data.poster}/>}
                        </Grid>
                    </Grid>
                </Box>
            </Modal>
